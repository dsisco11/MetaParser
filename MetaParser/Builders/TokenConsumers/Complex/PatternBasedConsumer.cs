...
namespace MetaParser.Builders.TokenConsumers.Complex;
...
internal class PatternBasedConsumer : IMetaCodeBuilder
{
    public void WriteTo(MetaParserContext context)
    {
        ...
        var depGraph = DepsGraph.Build(Tokens);
        ...
        // complex tokens all have start pattern seq. so do switch block here to detect pattern and switch to consumer func
        foreach (var token in tokenList)
        {
            Generate_Token_Consumption_Function(context, token);
        }
    }

    private static void Generate_Token_Consumption_Function(MetaParserContext context, DependencyNode<TokenDefComplex> token)
    {
        var wr = context.writer;
        ...
        {
            wr.WriteLine($"{CodeCommon.FormatSpanBuffer(context.IdType)} {nameEndTerminatorSeq} = stackalloc[] {{ {string.Join(", ", endTerminatorSeq)} }};");
        }

        if (endEscapeSeq is not null)
        {
            wr.WriteLine($"{CodeCommon.FormatSpanBuffer(context.IdType)} {nameEndEscapeSeq} = stackalloc[] {{ {string.Join(", ", endEscapeSeq)} }};");
        }
