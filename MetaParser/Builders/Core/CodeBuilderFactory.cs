...
//    public IMetaCodeBuilder Get_Switch_Return_For_Consumers() => new LogicConsumerSwitchReturn();
...
internal static class CodeBuilderFactory
{
    public static CodeBuilder If(string statement, CodeBuilder trueBuilder, CodeBuilder? falseBuilder = null)
    {
        return new IfElseCodeBuilder(statement, trueBuilder, falseBuilder);
    }

    public static CodeBuilder For(int start, int end, CodeBuilderCollection codeBuilders)
    {
        return new ForLoopCodeBuilder(start, end).AddWithin(codeBuilders);
    }

    public static CodeBuilder Method(IEnumerable<SyntaxToken> modifiers, TypeSyntax returnType, string name, ArgumentListSyntax arguments, CodeBuilderCollection codeBuilders)
    {
        return new MethodCodeBuilder(modifiers, returnType, name, arguments).AddWithin(codeBuilders);
    }
}
