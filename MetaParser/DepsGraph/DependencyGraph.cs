...
namespace MetaParser.DepsGraph
{
    internal class DependencyGraph
    {
        ...
        private void graph_node(TokenInfo token)
        {
            ...
            {
                graph_consumer((ParentNode)node, consumer);
            }
        }

        private void graph_consumer(ParentNode parent, ConsumerInfo consumer)
        {
            var node = new Node($"{consumer.Index}", parent);

            if (consumer.Start is not null)
            {
                graph_pattern(node, consumer.Start);
            }
            if (consumer.Consume is not null)
            {
                graph_pattern(node, consumer.Consume);
            }

            if (consumer.Stop is not null)
            {
                graph_pattern(node, consumer.Stop);

                if (consumer.Escape is not null)
                {
                    graph_pattern(node, consumer.Escape);
                }
            }
        }

        private void graph_pattern(Node node, PatternGroup patternGroup)
        {
            foreach (var subPattern in patternGroup.GetSubPatterns().OfType<PatternTokenRef>())
            {
                ...
                if (Nodes.TryGetValue(subPattern.TokenName, out var subTokDep))
                {
                    node.Link(subTokDep);
                }
            }
